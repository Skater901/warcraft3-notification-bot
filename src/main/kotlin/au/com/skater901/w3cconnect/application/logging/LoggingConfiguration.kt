...
class LoggingConfiguration : Configurator, ContextAwareBase() {
    ...
    companion object {
        ...
        @Inject
        @JvmStatic
        fun configure(configuration: WC3ConnectDiscordNotificationBotConfiguration) {
            consoleLogLevel = Level.toLevel(configuration.logging.consoleLoggingLevel, Level.OFF)
            fileLogLevel = Level.toLevel(configuration.logging.fileLoggingLevel, Level.INFO)
            logsDirectory = configuration.logging.logFileDirectory
            logFileArchiveCount = configuration.logging.logFileArchiveCount
        }
    }
}