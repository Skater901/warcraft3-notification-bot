...
internal class BattleNetGameProvider @Inject constructor(
    ...
    override suspend fun getGames(): List<Game> {
        val wc3StatsGames = emptyListOnException("wc3stats") {
            wc3StatsGamesWork {
        ...
        return emptyListOnException("wc3maps") {
            wc3MapsGamesWork {
    ...
    private suspend fun emptyListOnException(gamesSource: String, gamesProvider: suspend () -> List<Game>): List<Game> =
        try {
            gamesProvider()
        } catch (e: Exception) {
